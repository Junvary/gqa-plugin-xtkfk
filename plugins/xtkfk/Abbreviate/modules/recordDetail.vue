        ...
        <q-card style="width: 1400px; max-width: 80vw;">
            ...
            <q-card-section>
                <q-form ref="recordDetailForm">
                    ...
                    <div class="row">
                        <q-input class="col" v-model="recordDetail.value.top" label="大分类"
                            :rules="[val => val && val.length > 0 || '必须输入大分类']" />
                        <q-input class="col" v-model="recordDetail.value.from" label="来源" />
                    </div>
                    <q-field label="简称内容" stack-label>
                        <template v-slot:control>
                            <GqaEditor v-model="recordDetail.value.content" />
                        </template>
                    </q-field>
...
const url = {
    add: 'plugin-xtkfk/add-abbreviate',
    edit: 'plugin-xtkfk/edit-abbreviate',
    queryById: 'plugin-xtkfk/query-abbreviate-by-id',
}
const {
    dictOptions,
    showDateTime,
